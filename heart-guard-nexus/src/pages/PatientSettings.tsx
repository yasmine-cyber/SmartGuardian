...
import { Settings, Loader, Save, CheckCircle, User, Bell, Shield, Smartphone } from "lucide-react";
...
const PatientSettings = () => {
  ...
  const [activeTab, setActiveTab] = useState<"profil" | "medical" | "notifications" | "securite">("profil");
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (util) setProfile(util);

      const { data: patient } = await supabase
        .from("patients")
        .select("id, date_naissance, adresse, maladies, antecedents")
        .eq("user_id", user.id)
        .single();
      ...
      <div className="space-y-6 max-w-3xl">
        ...
        {loading ? (
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Les maladies sont modifiables depuis l'inscription
                    </p>
                  </div>
                  <div>
            ...
            {/* Bouton sauvegarder — sauf notifications et sécurité */}
